...
{   
	...
	//Pandemic module: switch is a global parameter, so should be ALSO set in agents (to allow as switch in stategraph)
	SWITCH_PANDEMIC_MODULE 		= CONST_SWITCH_PANDEMIC_MODULE;
	TRANSITION_PHASE_PANDEMIC	= CONST_TRANSITION_PHASE_PANDEMIC;	//Default: 8000 (400 months); this period is used for setting the starting point of any pandemic (activated after 400 months, so first time the policy is active is at the beginning of month 401)
	...
	INIT_TRANSIENT_PERIOD 		= CONST_INIT_TRANSIENT_PERIOD; 		//Default: 22 (day 2 of month 2); this period is used for setting the starting point of the Housing+Mortgage market (active from the start, basically)
	TRANSITION_PHASE_POLICY 	= CONST_TRANSITION_PHASE_POLICY;	//Default: 8000 (400 months); this period is used for setting the starting point of any policy (activated after 400 months, so first time the policy is active is at the beginning of month 401)
